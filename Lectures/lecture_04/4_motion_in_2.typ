...
#slide[
  - To prove the magnitude of the centripetal accelaration consider the diagram above then:
  $ arrow(v)= & v_x hat(i)+v_y hat(j)= -v sin(theta)+v cos(theta)j \
    =         & - frac(v y_p, r)hat(i)+frac(v x_p, r)hat(j) $
  to find the accelaration take time derivative
  $ arrow(a_c)= - frac(v^2 cos(theta), r)hat(i)+frac(- v^2 sin(theta), r)hat(j) $
  then the magnitude is simply $a_c=v^2 \/ r$.
]
...
#slide[ #figure(image("ss_16.png", width: 100%))]
#slide[
